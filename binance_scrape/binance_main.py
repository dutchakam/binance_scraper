...
def select_charts(page, bp: BinancePipeline) -> None:
    ...
    email_dataframe(pd.concat(frames))
...
@retry(OSError, tries=3)
def email_dataframe(df):
    ...
    with open(template_path, 'r', encoding='utf-8') as template_file:  # get the template from a text file
        ...
        mailer = Mailer(username=username, password=password, host=host, port=port)  # instantiate new Mailer class
        mailer.send_email(subject=subject, send_from=username, send_to=recipient, content=content, df=df)  # send email
